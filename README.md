...
ระบบแบ่งผู้ใช้งานออกเป็น 3 ระดับ โดยมีบัญชีทดสอบดังนี้:

1. **Admin (ผู้ดูแลระบบ)**
   - **ล็อกอิน:** `admin` / รหัสผ่าน `admin123`
   - **สิทธิ์:** เข้าถึงได้ทุกหน้า สามารถใช้ฟังก์ชัน Check-in, Check-out และดูแผนที่ส่วนกลาง (Live Map) ได้ทั้งหมด

2. **Dispatch (ผู้สั่งการ)**
   - **ล็อกอิน:** `dispatch` / รหัสผ่าน `dispatch123`
   - **สิทธิ์:** เข้าถึงได้เฉพาะหน้าแผนที่ (Live Map)
   - สามารถดูสถานะรถบนแผนที่, สั่งการเส้นทาง และกด Check-out ให้ชุดปฏิบัติการจากส่วนกลางได้
   - *ไม่สามารถ* เข้าหน้า Check-in เพื่อนำรถเข้าระบบด้วยตนเองได้

3. **Operator (ผู้ปฏิบัติการ)**
   - **ล็อกอิน:** `operator` / รหัสผ่าน `operator123` (หรือล็อกอินอัตโนมัติผ่านการสแกน QR Code)
   - **สิทธิ์:** เข้าถึงได้เฉพาะหน้า Check-in
   - สามารถทำ Check-in และ Check-out ให้ชุดปฏิบัติการของตนเองได้
   - *ไม่สามารถ* ดูหน้าแผนที่ส่วนกลาง (Live Map) ได้
